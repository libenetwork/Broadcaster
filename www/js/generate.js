...
webhookclient.addEventListener("message", e => {
    let data = JSON.parse(decodeURI(e.data)).name.replaceAll("+", " ") + " задон+атив " + JSON.parse(decodeURI(e.data)).amount + " гривень і сказав «" + JSON.parse(decodeURI(e.data)).message.replaceAll("+", " ").replaceAll("%2C", ",").replaceAll("%3B", ";").replaceAll("%3A", ":") + "»";
    generate(data, e)
})
